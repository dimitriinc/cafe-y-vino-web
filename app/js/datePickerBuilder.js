...
const date_element = document.getElementById('reserv-date')
...
function renderCalendar(month, year) {
    ...
    let day = 1
    const rows = calendarDates.getElementsByTagName('tr')
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            if (day <= monthLength && (i > 0 || j >= startingDay)) {
                ...
                // proceed only for the days that are not blocked
                if (!cells[j].classList.contains('day-blocked')) {
                    ...
                    if (day == selectedDay && currentMonth == selectedMonth && currentYear == selectedYear) {
                        ...
                        const momentDate = moment(new Date(selectedYear, selectedMonth, selectedDay))
